 ...
 * Desktop: standard table. Mobile (≤ 768px): each row collapses to a card
 * via CSS in assets/css/my-account.css; data-title attributes supply the
 * per-cell labels shown via the ::before pseudo-element.
 *
	...
	<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		...
		<tbody>
			...
			<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
				...
				<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) :
					$is_number = 'order-number' === $column_id;
					$tag       = $is_number ? 'th' : 'td';
					$scope     = $is_number ? ' scope="row"' : '';
				?>
				<<?php echo esc_attr( $tag ) . $scope; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
					...
					<?php elseif ( 'order-actions' === $column_id ) : ?>
						...
						?>
					<?php endif; ?>
				</<?php echo esc_attr( $tag ); ?>> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
...
<?php else : ?>
	...
	wc_print_notice(
		...
		. ' <a class="woocommerce-Button wc-forward button' . esc_attr( $wp_button_class ) . '" href="' . esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ) . '">'
